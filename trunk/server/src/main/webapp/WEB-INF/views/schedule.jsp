...
<script>
function openWin() {
    msgWindow= open("createlesson","createLesson",
     "width=530,height=230,status=no,toolbar=no,menubar=no,scrollbars=no")
   }
</script>  
<script>
function openWin1() {
    msgWindow= open("newschedule","newSchedule",
     "width=850,height=550,status=no,toolbar=no,menubar=no,scrollbars=no")
   }
</script>  
...
<form:form method="post" modelAttribute="date"   action="newData">
        ...
        <form:select path="iSMonth">
         ...
         <td width="2%"></td>
         	 <td colspan="2"><input type="submit" value="Enter Date" class="button"/></td> 
         	  <td width="2%"></td>
          <td colspan="2"> 
           <input type="button" value="Create Lesson" class="button"
                       onClick="openWin()">
          </td> 
<%--     </form:form>  --%>
<%--    	<form:form method="post" modelAttribute="dDate" action="delete"> --%>
			<td width="2%"></td>
			 <td colspan="2">
        ...
        <td width="2%"></td>
       	<td colspan="2"> 
           <input type="button" value="Create Schedule" class="button"
                       onClick="openWin1()">
        </td>
               ...
               <%}
                ...
                <td align="center"  width="14%"  id="day_<%=(cnt-newCalendar.getWeekStartDay()+1)%>">
                    ...
                    <% if(((cnt-newCalendar.getWeekStartDay()+1)>=1) ||((cnt-newCalendar.getWeekStartDay()+1)>=newCalendar.getDays()))
                           { %>
                         <tr>
                         <td colspan="3" id="it1"> <%=(cnt-newCalendar.getWeekStartDay()+1)%></td>
                         </tr>
                           <%} %>
                      <% int p=0;  
                          while ((f<newLessons.size())&&(p==0)){        
	                              if(newLessons.get(f).getData()==(cnt-newCalendar.getWeekStartDay()+1)){%> 
			            		    	  <tr id="it3"> 
<%--  										<td><a href="delete/<%=Integer.toString(newLessons.get(f).getNewLessonId())%>">delete</a></td> --%>
 			            		    	    <td><form:checkbox path="id" value="<%=Integer.toString(newLessons.get(f).getNewLessonId())%>"/></td>  
 										  ...
 										  <tr id="it4"> 
 										    <td colspan="2"><%=newLessons.get(f).getTeacherName()%></td> 
											<td><%=newLessons.get(f).getGroupName()%></td> 
										   </tr>	 
